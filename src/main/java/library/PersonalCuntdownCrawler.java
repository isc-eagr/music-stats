...
/**
 * Standalone crawler to scrape Vato's Cuntdown from classic.atrl.net
 * and output a SQL script for loading into the vatos_cuntdown_entry table.
 *
...
public class PersonalCuntdownCrawler {
    ...
    private void run() throws Exception {
        System.out.println("=== Vato's Cuntdown Crawler ===");
    ...
    * Writes a single-table import for vatos_cuntdown_entry only.
     */
    private void buildSql() throws Exception {
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(OUTPUT_FILE, StandardCharsets.UTF_8))) {
            pw.println("-- Vato's Cuntdown SQL import");
            ...
            pw.println();
            pw.println("-- ================================================================");
            pw.println("-- STEP 1: Import raw countdown entries into vatos_cuntdown_entry.");
            pw.println("-- ================================================================");
            pw.println();
            pw.println("BEGIN TRANSACTION;");
            pw.println();
            pw.println("DELETE FROM vatos_cuntdown_entry;");
            pw.println();

            for (RawEntry e : sortedEntries) {
                pw.printf("INSERT INTO vatos_cuntdown_entry (chart_date, position, artist_name, song_title, is_close_call, song_id) VALUES (%s, %d, %s, %s, %d, NULL);%n",
                    sqlStr(e.chartDate),
                    e.position,
                    sqlStr(e.artistName),
                    sqlStr(e.songTitle),
