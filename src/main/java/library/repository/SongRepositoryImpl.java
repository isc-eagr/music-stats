...
public class SongRepositoryImpl{
	...
	private static final String WHAT_WON_EACH_DAY = """
			select * from (select a.genre,a.date date_genre, max(a.duration) duration_genre,a.count count_genre from (select so.genre, date(sc.scrobble_date) date, sum(duration) duration, count(*) count 
			from song so inner join scrobble sc on so.id = sc.song_id 
			where date(sc.scrobble_date) >= '%s' and date(sc.scrobble_date) <= '%s' 
	...
	public List<WhatWonDTO> whatWonEachDay(String start, String end) {
		return template.query(WHAT_WON_EACH_DAY.formatted(start, end, start, end), new BeanPropertyRowMapper<>(WhatWonDTO.class));
	}
}
